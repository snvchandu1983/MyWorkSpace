...
jQuery(function() {
    ...
    $.fn.AdjustToScreen=function(level){
        var ul=this;
        ul.css('visibility','hidden');
        ul.css('display','block');//Can't get the location when display is set to none.
        var loc=Sys.UI.DomElement.getLocation(ul[0]);
        ul.css('visibility','');
        ul.css('display','none');
        var winWidth=$(window).width();
        var elWidth=ul.width();
        var overFlowWidth=parseInt(loc.x)+elWidth;
        if(overFlowWidth>winWidth) {
            var parentLi=ul.closest('li')[0];
            var liBounds=Sys.UI.DomElement.getBounds(parentLi).width;
            if(level==1) {
                ul.css('margin','-44px 0 0 -'+elWidth+'px');
            }
            else {
                elWidth-=liBounds;
                ul.css('margin','0 0 0 -'+elWidth+'px');
            }
        }
    }
    ...
    $('ul ul li', '#menu').hover(
        function(){
            ...
            if (ul.length){
                ...
                ul.css({width: 0, overflow: 'hidden'}).retarder(300, function(i){
...
//            $(this).parent().addClass('menuImgHover');
//        },
//        function(){
//            $(this).parent().removeClass('menuImgHover');
//        }
//    );
    // color animation
    if ($.browser.msie && $.browser.version.substr(0, 1) == '6'){
        $('ul ul a span', '#menu').css('color', 'rgb(255,255,255)').hover(
...
//            function(){ $(this).animate({color: 'rgb(255,255,255)'}, 500); },
             function(){ $(this).animate({color: 'rgb(0,0,0)'}, 500); },
