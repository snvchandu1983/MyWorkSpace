...
* Changed the signature of the SaveRow function and also its behaviour.*/ 
$.jgrid.extend({
//Editing
	...
	saveRow : function(rowid, succesfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc, onValidationError) {
		return this.each(function(){
		...
		if (editable==="1" && url) {
			...
			if (cv[0] === false){
				...
				if( $.isFunction(onValidationError) ) { onValidationError(rowId); }//Added by Danny to Hide progress if validation has occurred.
				return;
			}
