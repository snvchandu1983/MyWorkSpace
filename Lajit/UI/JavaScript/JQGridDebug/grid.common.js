...
*/ 
/*
Changes made with LAjit
function createEl() changed to accomodate colons in Value feild of DDLs
			...
			if(options.dataUrl != null) {
				...
				if(typeof options.value === 'string') {
					...
					for(i=0; i<so.length;i++){
						...
						//Modified by Danny to accomodate colons in the value field of the DDL 26/04/10
						if(sv.length>2) {
						    var newSv='';
						    for(var jnr=1;jnr<sv.length;jnr++) {
						        newSv+=sv[jnr];
						        if(jnr!=sv.length-1)
						            newSv+=':';
						    }
